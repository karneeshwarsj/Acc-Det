...
def estimate_video_probability(video_path: str) -> Dict[str, Any]:
    ...
    for frame_idx, frame in enumerate(frames):
        ...
        for i in range(len(tracked)):
            for j in range(i + 1, len(tracked)):
                ...
                # Use closing speed plus TTC, IoU, and trajectory convergence
                risk = compute_risk_score(
                    ttc,
                    iou,
                    traj,
                    relative_speed=max(0.0, rel_vel),
                )
    ...
    if not all_pair_risks:
        return {
            ...
            "label": "No Accident",
            "confidence": 1.0,
            "class_scores": {"Accident": 0.0, "No Accident": 1.0},
            "risk_level": "Low",
            "ttc_seconds": 999.0,
    ...
    accident_prob = agg["probability"]
    no_accident_prob = round(max(0.0, 1.0 - accident_prob), 4)
    ...
    return {
        ...
        "class_scores": {"Accident": round(accident_prob, 4), "No Accident": no_accident_prob},
