...
class ImageAccidentClassifier:
    ...
    def _load_hf_vit(self):
        """Load tiya1012/vit-accident-image from HuggingFace Hub."""
        from transformers import ViTForImageClassification, ViTImageProcessor

        print(f"[INFO] Loading {HF_MODEL_ID} from HuggingFace Hub (~330 MB, cached after first download)...")
        self.processor = ViTImageProcessor.from_pretrained(HF_MODEL_ID)
        self.model = ViTForImageClassification.from_pretrained(HF_MODEL_ID)
        self.model.to(self.device)
        self.model.eval()
        self.model_type = "vit_hf"
        self.model_status = "vit_pretrained_hf"
        print(f"✅ ViT accident classifier loaded (F1=0.93 on CCTV accident data)")
    ...
    def _load_efficientnet_fallback(self):
        """Load EfficientNet-B1 with ImageNet pretrained weights as fallback."""
        import timm
        print("[INFO] Using EfficientNet-B1 (ImageNet pretrained — lower accuracy)")
        ...
        self.model_status = "base_pretrained"
        self._build_efficientnet_transform()
